...
/**
 * 直接法的误差是标量，若只使用单个像素点的灰度值进行计算，则是一维，但是这里使用的是一个ｐａｔｃｈ
 * 使用的单个像素周围的１６个像素点的灰度值
 */
...
public:
    ...
    virtual void computeError() override {
        ...
        Vector3d pc=pose->estimate()*pw->estimate();
        float u=fx*pc[0]/pc[2]+cx;
        float v=fy*pc[0]/pc[2]+cy;
        error=
    ...
    virtual void linearizeOplus() override
    {
        ...
        float v = y * inv_z * fy + cy;//相机坐标转像素坐标
//d(u)/d(q)
...
//d(q)/d(kesi)
        ...
        for(int i = -2; i<2; i++)
            for(int j = -2; j<2; j++) {
                int num = 4 * i + j + 10;
                J_I_Puv(0,0) = (GetPixelValue(targetImg,u+i+1,v+j) - GetPixelValue(targetImg,u+i-1,v+j))/2;//ｕ方向梯度
                J_I_Puv(0,1) = (GetPixelValue(targetImg,u+i,v+j+1) - GetPixelValue(targetImg,u+i,v+j-1))/2;//v方向梯度
                _jacobianOplusXi.block<1,3>(num,0) = -J_I_Puv * J_Puv_Pc * vertexTcw->estimate().rotation_matrix();//误差对地图点的导数
                _jacobianOplusXj.block<1,6>(num,0) = -J_I_Puv * J_Puv_Pc * J_Pc_kesi;//误差对位姿的偏导
            }
    }
